...
public class hamilton {
	...
	static String[] next(ArrayList s,int l,int len,String t[]) {
		String[]tmp=new String[5];int tl;int k;
		for(int q=0;q<t.length;q++) {
		System.out.println(t);
		
		 tl=t[q].length();//tl-l+1;
		k=0;
		...
		String st;
			...
			for(int j=0;j<len;j++) {
				...
				if(t[q].substring(tl-l+1).equals(st)) {
					tmp[k]=t+stmp.substring(l-1);k++;
				}
